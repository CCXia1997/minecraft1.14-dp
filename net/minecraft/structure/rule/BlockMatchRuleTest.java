...
public class BlockMatchRuleTest extends AbstractRuleTest
{
    ...
    public BlockMatchRuleTest(final Block block) {
        this.block = block;
    }
    
    public <T> BlockMatchRuleTest(final Dynamic<T> dynamic) {
    ...
    protected <T> Dynamic<T> serialize(final DynamicOps<T> dynamicOps) {
        return (Dynamic<T>)new Dynamic((DynamicOps)dynamicOps, dynamicOps.createMap((Map)ImmutableMap.of(dynamicOps.createString("block"), dynamicOps.createString(Registry.BLOCK.getId(this.block).toString()))));
    }
}
