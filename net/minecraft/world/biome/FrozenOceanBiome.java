...
public final class FrozenOceanBiome extends Biome
{
    ...
    public FrozenOceanBiome() {
        super(new Settings().<TernarySurfaceConfig>configureSurfaceBuilder(SurfaceBuilder.FROZEN_OCEAN, SurfaceBuilder.GRASS_CONFIG).precipitation(Precipitation.SNOW).category(Category.OCEAN).depth(-1.0f).scale(0.1f).temperature(0.0f).downfall(0.5f).waterColor(3750089).waterFogColor(329011).parent(null));
    ...
    public float getTemperature(final BlockPos blockPos) {
        ...
        final double double3 = FrozenOceanBiome.u.sample(blockPos.getX() * 0.05, blockPos.getZ() * 0.05);
        final double double4 = FrozenOceanBiome.FOLIAGE_NOISE.sample(blockPos.getX() * 0.2, blockPos.getZ() * 0.2);
        final double double5 = double3 + double4;
        if (double5 < 0.3) {
            final double double6 = FrozenOceanBiome.FOLIAGE_NOISE.sample(blockPos.getX() * 0.09, blockPos.getZ() * 0.09);
            if (double6 < 0.8) {
                float2 = 0.2f;
            }
        }
        if (blockPos.getY() > 64) {
            final float float3 = (float)(FrozenOceanBiome.TEMPERATURE_NOISE.sample(blockPos.getX() / 8.0f, blockPos.getZ() / 8.0f) * 4.0);
