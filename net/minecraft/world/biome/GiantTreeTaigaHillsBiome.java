...
public final class GiantTreeTaigaHillsBiome extends Biome
{
    public GiantTreeTaigaHillsBiome() {
        super(new Settings().<TernarySurfaceConfig>configureSurfaceBuilder(SurfaceBuilder.GIANT_TREE_TAIGA, SurfaceBuilder.GRASS_CONFIG).precipitation(Precipitation.RAIN).category(Category.TAIGA).depth(0.45f).scale(0.3f).temperature(0.3f).downfall(0.8f).waterColor(4159204).waterFogColor(329011).parent(null));
        ...
        this.addSpawn(EntityCategory.a, new SpawnEntry(EntityType.ZOMBIE, 100, 4, 4));
        this.addSpawn(EntityCategory.a, new SpawnEntry(EntityType.SKELETON, 100, 4, 4));
        this.addSpawn(EntityCategory.a, new SpawnEntry(EntityType.ZOMBIE_VILLAGER, 25, 1, 1));
