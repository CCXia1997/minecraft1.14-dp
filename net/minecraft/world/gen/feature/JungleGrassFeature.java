...
{
    public JungleGrassFeature(final Function<Dynamic<?>, ? extends DefaultFeatureConfig> configDeserializer) {
        super(configDeserializer);
    }
    
    public BlockState getGrass(final Random random) {
        return (random.nextInt(4) == 0) ? Blocks.aR.getDefaultState() : Blocks.aQ.getDefaultState();
    }
    ...
    public boolean generate(final IWorld world, final ChunkGenerator<? extends ChunkGeneratorConfig> generator, final Random random, BlockPos pos, final DefaultFeatureConfig config) {
        ...
        for (int integer9 = 0; integer9 < 128; ++integer9) {
            ...
            if (world.isAir(blockPos10) && world.getBlockState(blockPos10.down()).getBlock() != Blocks.l && blockState6.canPlaceAt(world, blockPos10)) {
