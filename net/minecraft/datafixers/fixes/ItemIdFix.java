...
{
    private static final Int2ObjectMap<String> NUMERICAL_ID_TO_STRING_ID_MAP;
    
    public ItemIdFix(final Schema schema, final boolean boolean2) {
        super(schema, boolean2);
    }
    
    public static String fromId(final int integer) {
        return (String)ItemIdFix.NUMERICAL_ID_TO_STRING_ID_MAP.get(integer);
    }
    
    public TypeRewriteRule makeRule() {
        final Type<Either<Integer, Pair<String, String>>> type1 = (Type<Either<Integer, Pair<String, String>>>)DSL.or(DSL.intType(), DSL.named(TypeReferences.ITEM_NAME.typeName(), DSL.namespacedString()));
        final Type<Pair<String, String>> type2 = (Type<Pair<String, String>>)DSL.named(TypeReferences.ITEM_NAME.typeName(), DSL.namespacedString());
        final OpticFinder<Either<Integer, Pair<String, String>>> opticFinder3 = (OpticFinder<Either<Integer, Pair<String, String>>>)DSL.fieldFinder("id", (Type)type1);
        return this.fixTypeEverywhereTyped("ItemIdFix", this.getInputSchema().getType(TypeReferences.ITEM_STACK), this.getOutputSchema().getType(TypeReferences.ITEM_STACK), typed -> typed.update((OpticFinder)opticFinder3, (Type)type2, either -> (Pair)either.map(integer -> Pair.of(TypeReferences.ITEM_NAME.typeName(), fromId(integer)), pair -> pair)));
    }
    
    static {
        NUMERICAL_ID_TO_STRING_ID_MAP = (Int2ObjectMap)DataFixUtils.make(new Int2ObjectOpenHashMap(), int2ObjectOpenHashMap -> {
