...
{
    private static final Int2ObjectMap<String> NUMERICAL_KEY_IDS_TO_KEY_NAMES;
    ...
    public TypeRewriteRule makeRule() {
        ...
        String string4;
        String string5;
        return this.fixTypeEverywhereTyped("OptionsKeyLwjgl3Fix", this.getInputSchema().getType(TypeReferences.OPTIONS), typed -> typed.update(DSL.remainderFinder(), dynamic -> dynamic.getMapValues().map(map -> dynamic.createMap((Map)map.entrySet().stream().map(entry -> {
            if (entry.getKey().asString("").startsWith("key_")) {
                integer2 = Integer.parseInt(((Dynamic)entry.getValue()).asString(""));
                if (integer2 < 0) {
                    ...
                    string5 = (String)OptionsKeyLwjgl3Fix.NUMERICAL_KEY_IDS_TO_KEY_NAMES.getOrDefault(integer2, "key.unknown");
    ...
    static {
        NUMERICAL_KEY_IDS_TO_KEY_NAMES = (Int2ObjectMap)DataFixUtils.make(new Int2ObjectOpenHashMap(), int2ObjectOpenHashMap -> {
